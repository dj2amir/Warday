    ...
    if (!m_caster->HasUnitMovementFlag(MOVEMENTFLAG_FALLING) || (pos.GetPositionZ() - ground < distance))
    {
        ...
        for (; j < (numChecks + 1); j++)
        {
            ...
            if ((!map->IsInWater(tstX, tstY, tstZ) && tstZ != beforewaterz) || wcol)  // second safety check z for blink way if on the ground
            {
                ...
                tstZ1 = map->GetHeight(phasemask, tstX, tstY, prevZ + maxtravelDistZ, true, 10.0f);
                // upper or floor
                tstZ2 = map->GetHeight(phasemask, tstX, tstY, prevZ, true, 10.0f);
                //lower than floor
                tstZ3 = map->GetHeight(phasemask, tstX, tstY, prevZ - maxtravelDistZ / 2, true, 5.0f);
            ...
            if (col || dcol || (overdistance > 0.0f && !map->IsInWater(tstX, tstY, ground)) || (fabs(prevZ - tstZ) > maxtravelDistZ && (tstZ > prevZ)))
            {
                ...
                //lower than floor
                destz3 = map->GetHeight(phasemask, destx, desty, prevZ - maxtravelDistZ / 2, true, 5.0f);
